...
to setup
  ...
  ;; asignacion de icono de persona
  ask equipoA [ set shape "person" ]
...
to mundo-1
  ...
  ask patches [
    if (pxcor >= 2 and pycor > 0) [ set pcolor blue ]
  ]

  ask patches [
    if (pxcor <= -2 and pycor > 0) [ set pcolor red ]
  ]

  ;; limites externos
  ask patches [
    if (pxcor = 16) [ set pcolor black ]
  ]
  ask patches [
    if (pxcor = -16) [ set pcolor black ]
  ]
  ask patches [
    if (pycor = 16) [ set pcolor black ]
  ]
  ask patches [
    if (pycor = -16) [ set pcolor black ]
  ]
  ...
  ;; muro interno
  ask patches [
    if (pxcor > -2 and pxcor < 2 and pycor > 0) [ set pcolor black ]
  ]
end
...
to regla-equipo-A
  ;; Debe salir de la zona azul
  if pcolor = blue [
    face min-one-of patches with [ pcolor = white ] [ distance myself ]
    fd 1
    regla-general
  ]
end
...
to regla-equipo-B
  ...
  ;; si se entra en la zona azul, se esta a salvo
  if pcolor = blue [
    set aSalvo (aSalvo + 1)
    die
  ]
...
to paso
  ;; se gira al azar dentro del angulo de vision
  right random 130
  ;; se da un paso al frente
  fd 1
  regla-general
end
...
to regla-cazador
  ...
  let corx xcor
  let cory ycor
  ;; se busca la presa mas cercana en un rango de 3 patches y 130 grados
  ask equipoB in-cone 3 130 [
    ;; en caso de encontrarla se asignan nuevas variables
    set corx xcor
    set cory ycor
  ]
...
to alejar
  ...
  ask equipoA in-cone 4 130[
    ;; en caso de encontrar un cazador cerca, se asignan nuevas coordenadas.
    set corx xcor
    set cory ycor
  ]
  ...
  [
    ;; si hay algun cazador cerca, se aumenta el miedo en 2 unidades
    set miedo (miedo + 2)
    alejar-por-miedo
    regla-general
    right random 180
...
to alejar-por-miedo
  ifelse miedo > 1
  [
    ;; se busca el cazador más cercano
  ...
  [
    ;; se busca la zona segura(zona de color azul)
    face min-one-of patches with [ pcolor = blue ] [ distance myself ]
    ;; se da un paso en esa dirección
    fd 1
    regla-general
...
to dilema-puercoespin-equipoA
  ...
  let corx xcor
  let cory ycor
  ;; se buscan integrantes del mismo equipo cerca
  ask equipoA in-cone 1 130 [
    ;; de encontrarlos se sobre escriben las coordenadas
    set corx xcor
    set cory ycor
  ]
...
http://www.netlogoweb.org/launch#http://www.netlogoweb.org/assets/modelslib/Sample%20Models/Art/Follower.nlogo

http://ccl.northwestern.edu/netlogo/docs/dict/one-of.html

http://ccl.northwestern.edu/netlogo/docs/dict/turtles-at.html
